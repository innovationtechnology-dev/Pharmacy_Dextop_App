...
export interface CreateCloudLicenseRequestPayload {
  pharmacyName: string;
  email: string;
  phone: string;
  doctorName?: string;
...
export interface CreateCloudLicenseRequestResult {
  ok: boolean;
  id?: string;
  error?: string;
}

export const createCloudLicenseRequest = async (
  payload: CreateCloudLicenseRequestPayload
): Promise<CreateCloudLicenseRequestResult> => {
  try {
    const baseUrl = getCloudLicenseApiBaseUrl();
    const res = await fetch(`${baseUrl.replace(/\/+$/, '')}/api/license-requests`, {
      method: 'POST',
      headers: {
    ...
    if (!data.ok || !data.id) {
      return {
        ok: false,
        error: data.error || 'Failed to create license request',
      };
    }
    ...
    return {
      ok: false,
      error: 'Unable to send license request. Check URL or internet connection.',
    };
  }
};
